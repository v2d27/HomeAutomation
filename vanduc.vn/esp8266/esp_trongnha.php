<?php
    ...
    mysqli_query($connection, "UPDATE role SET last_get_times = '$times GMT+7' WHERE STT = 2");
    exit;
    */
    ...
    // on alarm: not sent: 0
    // on alarm:  is sent: 1
    //off alarm: not sent: 2
    //off alarm:  is sent: 3

    /*
    //Alarm on/off state in esp8266
    $current_hour = substr($times, 11, 2);
    $current_min  = substr($times, 14, 2);
    echo "Debug: hour  = " . $current_hour. ", min = " . $current_min;
    $current_time = intval($current_hour)*60 + intval($current_min);
    echo "Current time:" . $current_time;

    $on_time = intval(substr($db_on, 0, 2))*60 + intval(substr($db_on, 3, 2));
    $off_time = intval(substr($db_off, 0, 2))*60 + intval(substr($db_off, 3, 2));
    if ($current_time >= ) {
        # code...
    }
    */
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if(intval($role_id) > 10)
            {
                ...
                if (alarm_check($row['on_time'], $row['off_time'])) {
                    if ($arr[$role_str] == "0") {
                        $arr[$role_str] = "1";
                        mysqli_query($connection, "UPDATE role SET role$role_id=1 WHERE STT = 2");
                    }
                    if ($row['STT'] == "0") {
                        $role_id += 20;
                ...
                else
                {
                    if ($arr[$role_str] == "1") {
                        ...
                        {
                            $arr[$role_str] = "0";
                            mysqli_query($connection, "UPDATE role SET role$role_id=0 WHERE STT = 2");
                            $role_id += 20;
